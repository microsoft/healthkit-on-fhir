...
open class AppleStandTimeContainer : ResourceContainer<Observation>, HDSExternalObjectProtocol {
    internal let unit = "ms"
    
    public static func authorizationTypes() -> [HKObjectType]? {
        if let appleStandTime = healthKitObjectType() {
    ...
    public static func externalObject(object: HKObject, converter: HDSConverterProtocol?) -> HDSExternalObjectProtocol? {
        if let sample = object as? HKSample,
            sample.sampleType == AppleStandTimeContainer.healthKitObjectType() {
            return AppleStandTimeContainer(object: object, converter: converter)
        }
    ...
    public static func externalObject(deletedObject: HKDeletedObject, converter: HDSConverterProtocol?) -> HDSExternalObjectProtocol? {
        return AppleStandTimeContainer(deletedObject: deletedObject, converter: converter)
    }
